...
- **Toggle** the calculator with the `C` key (rebindable) while any GUI is open
- **Live preview** — displays `= result` in real time as you type an expression
- **Press Enter** to evaluate and replace the expression (supports chained calculations)
...
- **Supported operators**: `+` `-` `*` `/` `^` `( )` `.` and Unicode `×` `÷`
